...
module = Module('reaver', logging.DEBUG)
job_manager = JobManager(name='reaver', module=module, log_level=logging.DEBUG)

history_directory_path = '/root/.reaver'
...
class ScanJob(Job[bool]):
    ...
    def do_work(self, logger: logging.Logger) -> bool:
        ...
        logger.debug(f'Calling reaver and writing output to {self.output_file}')
...
def _notify_dependencies_finished(job: OpkgJob):
    if not job.was_successful:
        ...
        module.send_notification('Reaver finished installing.', notifier.INFO)
...
def check_dependencies(request: Request):
    return opkg.check_if_installed('reaver', module.logger)


@module.handles_action('manage_dependencies')
def manage_dependencies(request: Request):
    return {
        'job_id': job_manager.execute_job(OpkgJob('reaver', request.install), callbacks=[_notify_dependencies_finished])
    }
